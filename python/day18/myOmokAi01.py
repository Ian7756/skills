...
class MyWindow(QMainWindow):
    ...
    def setOmockDb(self,o_history_list,win):
        ...
        for i,o_history in enumerate(o_history_list):
            sql = "insert into omockai(pan,o_seq,o_history,win) values ("+str(pan)+","+str(i+1)+",'"+o_history+"',"+str(win)+")"
            curs.execute(sql)
        conn.commit()
        conn.close()
    ...
    def getPan(self):
        conn = pymysql.connect(host='127.0.0.1', user='root', password='java', db='mypy', charset='utf8')
        curs = conn.cursor()
        sql = "select COALESCE(max(pan),0)+1 from omockai"
        curs.execute(sql)
        rows = curs.fetchall()
        pan = rows[0][0] 
        conn.close()
    ...
    def goCompTurn(self):
        ...
        int_i, int_j = self.getComMapIJFromAI(str(self.arr2d).replace(".","").replace(" ","").replace("\n","").replace("[","").replace("]",""))
    ...
    def getComMapIJFromAI(self,str_info400):
